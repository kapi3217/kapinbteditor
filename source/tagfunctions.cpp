...
string ReadString(const vector<uint8_t> &data, int &i) {
...
void ConvertPartToLittleEndian(vector <uint8_t>& data) {
    int it = data.size() / 8;
    for (int j = 0; j < it; j++) { // For each word
        for (int k = 0; k < 4; k++) { // Change order from big-endian to little-endian
            swap(data[j * 8 + k], data[j * 8 + 7 - k]);
        }
    }
}
